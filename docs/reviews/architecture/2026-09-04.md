...
**Resolution (AR-005):** Resolved. The lifecycle is now restartable: workers
are stored as zero-argument *factories* in `log_worker_factories` and invoked
by `start_logging` to schedule fresh tasks each cycle; `stop_logging` no longer
calls `close()` and is idempotent. Guard semantics are explicit and documented
in `docs/architecture/lifecycle.md`: double-start raises `RuntimeError`,
stop-without-start is a no-op, `emit` between stop and next start is a silent
no-op, and restart is supported only on the same event loop (cross-loop restart
requires reconstructing the handler).
...
**Resolution (AR-007):** Resolved. Backend queues are now bounded by
`queue_maxsize`, a keyword-only constructor parameter on `AsyncLoggingHandler`
and `AsyncBaseHandler` (default 10000), stored as `self.queue_maxsize`.
`ConsoleBackend` builds `asyncio.Queue(maxsize=maxsize)` and `AsyncBrokerHandler`
builds `asyncio.Queue(maxsize=self.queue_maxsize)`; `queue_maxsize` flows through
`**kwargs` to Redis/Valkey handlers unchanged. The overflow policy is **drop +
report**: when a backend queue is full at emit time, `emit` drops the record and
routes an `asyncio.QueueFull` to the error channel (`_report_error` →
`error_handler` callback or module logger). `emit` never blocks, so the producer
stays non-blocking under sustained overload. The `QueueFull` branch is now
reachable and explicit. `ConsoleBackend.drain` uses `put_nowait` for its
synthetic shutdown-status records, dropping them if the console queue is full so
shutdown never deadlocks on a bounded queue. Documented in
`docs/configuration.md` and `docs/architecture/data-flow.md`.
